...
class Postgres {
    ...
    async ensureSchema() {
        ...
        -- allow pseudo caching of stickers to reduce GC calls
        CREATE TABLE IF NOT EXISTS stickers (
            stickerid   smallint NOT NULL, 
            defindex    smallint NOT NULL,
            stickers    jsonb,
            rarity      smallint NOT NULL,
        ...
        // pseudo unique -> not per-user unique (by assedId), but for now, this is enough to reduce num of GC request
        await this.pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS i_unique_stickerid ON stickers (stickerid)`);
    }

    async insertItemData(item, price) {
        winston.debug(`insertItemData() called for ${item.defindex} with enableBulkInserts set to ${this.enableBulkInserts}`);
    ...
    // Stickers: paintindex: "0", defindex: "1209" (equals item type Sticker) => then stickerId (e.g. "2638") is part of sticker_kits
    ...
    async handleBulkInsert(data) {
        ...
        for (let [item, price] of data) {
            ...
            winston.debug(`handleBulkInsert() called for ${item.defindex}`);

            if (item.floatvalue <= 0 && item.defindex !== (507 || 1209)) {
                // Only insert weapons and stickers, naive check
            ...
            // TODO if sticker (defindex 1209) insert to stickers table
